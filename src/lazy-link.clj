...
(defn process-matches-when-match
  ...
  (when (matches? L R)
    (add-to-all 
     (lazy-seq (sb (inc a) (rest L) sl (rest R)))
     {:a a :b b :type (first L)})))


(defn process-matches-when-no-match
  [a L sr R]
  (lazy-seq (sb (inc a) L sr R)))
...
(defn process-poss 
  [idx L sen R i disjs]
  (apply concat
   (for [d disjs :when (right-size? (inc i) (- (count sen) i) d)]
     (process-sub-matches idx L sen R i d))))
...
(defn process-non-prime 
  ...
  (apply concat
	 (for [i (range (count sen)) 
	   ...
	   (process-poss idx L sen R i middle-word))))
...
(defn sb [idx L sen R]
  (cond
    ...
    :otherwise
    (process-non-prime idx L sen R 0)))
...
(defn process-djs [sen]
  (apply concat
	 (let [ss (subvec sen 1)
	   ...
	   (for [d djs
		:when (nil? (seq (:left d)))]
	    (process-matches-when-no-match 0 (:right d) ss nil)))))
